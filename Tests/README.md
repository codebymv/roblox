...
Covers `LabConfig` invariants, `RouteMath` arc-length maths, `TokenBucket`
arithmetic, `TuningSchema` consistency, and the `MatchConfig` leg ladder. Lune
...
Covers what only the engine can answer: that the remotes exist, that the world
